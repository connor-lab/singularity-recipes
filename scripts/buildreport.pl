...
# Script to take a sequence file in fasta format, use this to query Stanford via serrapy
# a JSON file is then generated by sierraypy from the Stanford database, which is parsed and rerendered it into 
# a RTF document using a standard template that has been agreed with clinical scientists and medical staff
...
# now we have the elements out of the JSON we can write the RTF itself
...
# Add header info with page numbers

$rtf->number_pages("PHW HIV Report for sample $sample, p.");
...
# Elements of the report are all in tables to enable us to provide a consistent display within the constraints of an RTF
